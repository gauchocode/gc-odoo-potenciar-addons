.. |badge-repo| image:: https://img.shields.io/badge/github-gauchocode%2Fgc--odoo--potenciar--addons-lightgray.png?logo=github
   :target: https://github.com/gauchocode/gc-odoo-potenciar-addons/tree/16.0/gc_odoo_sale_invoice_import
   :alt: gauchocode/gc-odoo-potenciar-addons - GitHub
...
para cada tipo de importación (ver sección `Import types`_).
...
#. Seleccionar el archivo Excel (.xlsx).
#. Elegir el **tipo de importación** según el formato del archivo.
#. Seleccionar la **hoja** a procesar (se listan automáticamente al cargar el archivo).
#. Revisar el **preview**: cada fila muestra su estado (*Lista*, *Duplicada*, *Error*).
#. Presionar **Importar** para crear las facturas en borrador.
...
- El encabezado de cada columna identifica el **plan analítico**
  (ej. ``Analítico DOC`` → plan ``DOC``).
- El valor de la celda se busca en ``account.analytic.account`` por nombre exacto,
  código o display name. Si el plan está identificado, la búsqueda se restringe a ese plan.
- Los analíticos encontrados se asignan al **100%** en cada línea
...
Los códigos de producto se extraen automáticamente del encabezado entre corchetes
(ej. ``Comisión [000010]`` → producto ``000010``).
...
- La columna Q genera una línea con IVA **Exento** (impuesto con nombre ``IVA EXENTO``).
- La columna R genera una línea con IVA **21%**.
- Se valida que ``EXENTO + GRAVADO + IVA = TOTAL`` dentro de la tolerancia de redondeo.
- Fallback de productos si el encabezado no trae código: ``000005`` (Q) y ``000012`` (R).
...
20 columnas. Misma estructura y lógica que CPD (validación de total incluida).
...
.. list-table::
   ...
   * - 17
     - Q
     - Precio Neto Grav Anual
     - IVA 21%
   * - 18
     - R
     - Precio Neto Grav Mensual
     - IVA 21%
   * - 19
     - S
     - Precio L Producto [000012]
     - IVA 21%
   * - 20
     - T
     - IVA (21%)
...
- Fallback de productos: Q → ``000003``, R → ``000004``.
- El código de la columna S se extrae del encabezado.
...
código del encabezado de S, impuesto IVA 21%, cuentas analíticas K–N.
...
.. list-table::
   ...
   * - 17
     - Q
     - EXENTO Comision [código]
     - IVA Exento
   * - 18
     - R
     - GRAVADO Liq Prod [código]
     - IVA 21%
   * - 19
     - S
     - GRAVADO Rpro Gtos [código]
     - IVA 21%
   * - 20
     - T
     - IVA (21%)
...
- Las columnas R y S generan líneas con IVA **21%** (S solo si tiene importe > 0).
...
* `GauchoCode <https://github.com/gauchocode>`_
